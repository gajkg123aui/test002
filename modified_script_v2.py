...
def split_lines_advanced(alist, image_height, threshold_scale=2.0, min_line_height=5):
    """
    Splits lines using a combination of Y-axis projection and statistical gap analysis.
    """
    if not alist:
        return alist

    # 1. Y-Axis Projection Profile
    y_projection = np.zeros(image_height)
    for char_data in alist:
        # y-coordinates are at indices 4 and 6
        y0, y1 = int(char_data[4]), int(char_data[6])
        # Ensure y0 is less than y1
        if y0 > y1:
            y0, y1 = y1, y0
        y_projection[y0:y1] += 1

    # Find line boundaries by detecting valleys (zero-projection areas)
    in_line = False
    line_boundaries = []
    line_start = 0
    for y, proj_val in enumerate(y_projection):
        if proj_val > 0 and not in_line:
            in_line = True
            line_start = y
        elif proj_val == 0 and in_line:
            in_line = False
            if y - line_start > min_line_height: # Filter out noise
                line_boundaries.append((line_start, y))

    if in_line and image_height - line_start > min_line_height: # Add last line if it extends to the end
        line_boundaries.append((line_start, image_height))

    if not line_boundaries:
        return alist # No lines found

    # 2. Assign characters to initial lines based on projection
    for char_data in alist:
        char_y_center = char_data[8] # center-y is at index 8
        line_assigned = False
        for i, (top, bottom) in enumerate(line_boundaries):
            if top <= char_y_center < bottom:
                char_data[21] = i  # Use index 21 for the new line ID
                line_assigned = True
                break
        if not line_assigned:
            char_data[21] = -1 # Mark as unassigned

    # Group characters by the new line ID
    lines = defaultdict(list)
    for char_data in alist:
        if char_data[21] != -1:
            lines[char_data[21]].append(char_data)

    # 3. Refine splits using statistical gap analysis
    final_alist = []
    new_line_id_counter = 0
    ...
    for line_key in sorted_line_keys:
        ...
        # Sort characters in the line by x-coordinate
        line.sort(key=lambda x: x[7]) # center-x is at index 7

        # Analyze horizontal gaps
        gaps = []
        for i in range(len(line) - 1):
            gap = line[i+1][7] - line[i][7] # Gap between center x
            if gap > 0:
                gaps.append(gap)

        if not gaps:
            # If no gaps, treat as a single line
            for char_data in line:
                char_data[14] = line[0][3] # New line x0 (index 3)
                char_data[15] = line[0][4] # New line y0 (index 4)
                char_data[21] = new_line_id_counter # Final line ID
            final_alist.extend(line)
            new_line_id_counter += 1
            continue

        avg_gap = np.mean(gaps)
        std_gap = np.std(gaps)
        avg_char_width = np.mean([c[9] for c in line]) # width is at index 9

        # Split threshold: combination of statistical gap and average char width
        split_threshold = max(avg_gap + threshold_scale * std_gap, avg_char_width * 1.5)

        current_sub_line_start_index = 0
        for i in range(len(line) - 1):
            gap = line[i+1][7] - line[i][7]
            if gap > split_threshold:
                # Found a split point, finalize the previous sub-line
                sub_line = line[current_sub_line_start_index : i+1]
                for char_data in sub_line:
                    char_data[14] = sub_line[0][3]
                    char_data[15] = sub_line[0][4]
                    char_data[21] = new_line_id_counter
                final_alist.extend(sub_line)
                new_line_id_counter += 1
                current_sub_line_start_index = i + 1

        # Add the last sub-line
        last_sub_line = line[current_sub_line_start_index:]
        if last_sub_line:
            for char_data in last_sub_line:
                char_data[14] = last_sub_line[0][3]
                char_data[15] = last_sub_line[0][4]
                char_data[21] = new_line_id_counter
            final_alist.extend(last_sub_line)
            new_line_id_counter += 1
...
def sort_vlist1(zlist):
    ...
    if len(zlist)>0:
        ...
        zhlist = sorted(zhlist, key=lambda x: (int(x[22]), int(x[8]), int(x[7])))
        zlist = zvlist+zhlist
        return zlist
            ...
            for ifile in sorted_list:
                ...
                if plan == "A シングル":
                    ...
                    if os.path.isfile(azure_name) is True:
                        ...
                        ### 市民タイムス行分割処理ここから ###
                        alist = split_lines_advanced(alist, image_height)
                        ### 市民タイムス行分割処理ここまで ###
