...
def sort_vlist1(zlist):
    """
    新聞のレイアウトを解析し、テキストを正しい読み順にソートします。
    OCR元画像が90度回転されていることを前提とします。
    - pandas DataFrameを使用して、複雑なソートを効率的に行います。
    - 縦書きと横書きのテキストブロックを統合的に扱います。
    - ブロックをまず「段」（元画像の列）にグループ化し、次に段を上から下へソートします。
    - 各段の中のテキストは左から右へソートします。
    """
    if not zlist:
        return []
    ...
    # データ型を適切なものに変換
    for col in ['x0', 'y0', 'x1', 'y1', 'mx', 'my', 'w', 'h', 'line_x0', 'line_y0', 'line_x1', 'line_y1', 'gno']:
        df[col] = pd.to_numeric(df[col], errors='coerce')

    # --- 段組の特定 (90度回転後の座標系に基づく) ---
    # 元画像の「列」は、回転後にはY座標でグループ化できる「行のようなブロック」になる
    # Y座標の中心(my)でソートし、Y座標の差が閾値より大きい場合に新しい段組IDを割り当てる
    df = df.sort_values(by='my')
    # 段組の幅（回転後は高さ）は新聞によって異なるため、閾値は調整が必要
    # ここでは平均的な文字の高さの数倍を想定
    ...
    # 各段組の代表Y座標を計算（ソートのため）
    col_positions_y = df.groupby('col_id')['my'].mean()
    df['col_pos_y'] = df['col_id'].map(col_positions_y)

    # --- ソート処理 ---
    # 1. 段組を上から下へソート (col_pos_y の昇順)
    # 2. 段組の中で、テキストを左から右へソート (mx の昇順)
    # 3. 念のため、元の行・単語順もキーに追加

    df = df.sort_values(
        by=['col_pos_y', 'mx', 'line_in_block', 'word_in_line'],
        ascending=[True, True, True, True]
    )

    # DataFrameをリストに戻す
    sorted_list = df.drop(columns=['col_id', 'col_pos_y'], errors='ignore').values.tolist()
                    ...
                    if os.path.isfile(azure_name) is True:
                        ...
                        for l in range(0, len(alist), 1):
                            ...
                            if  (alist[l][1]==1 or 6<=alist[l][1]) and alist[l][9]<=30 and alist[l-1][13] == 'horizontal'and alist[l][13] == 'horizontal':
                                ...
                                #スペース
                                elif (abs(alist[l-1][1]+alist[l-1][2]-alist[l][1])>=2 and alist[l][1]!=1 and not alist[l][0] in '、()（）「」"“…・' and not alist[l-1][0] in '、()（）「」"“…・') or int(alist[l][24])>=38: #
                                    print('space'+ str(l))
                                    for n in range(l, len(alist), 1):
                                        if l == n:
                                            ...
                                            if alist[n][1]==1 or (not alist[n][0] in '、()（）「」"“…・' and int(alist[n][24])>=38):
                                                l=n-1
                                                o=0
                                                break
                                            elif m1+m2+p<=alist[n][1]+alist[n][2] and alist[n][14] == lx and alist[n][15] == ly:
                                                m2 = alist[n][2]
                                                if m2>0 :
