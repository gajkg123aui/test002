...
import numpy as np
import pymupdf
# Pylance may report false-positive errors for pymupdf attributes
# like get_pixmap, insert_text, and set_metadata. These are known
# type-hinting issues and the code is correct.
...
#CONFIG_INI.read('/home/takesawa/NAS_TEST/ocr_key3.ini', 'UTF-8') #絶対パス

# 指定したiniファイルが存在しない場合、エラー発生
#if not os.path.exists('/home/takesawa/NAS_TEST/ocr_key3.ini'):  #絶対パス
    # raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), '/home/takesawa/NAS_TEST/ocr_key3.ini')  #絶対パス
...
DPI = int(CONFIG_INI['DEFAULT']['_dpi'])
...
def azure(aifile, bifile):
    """
    aaa
    """
    file = aifile.getvalue()
    content_type = "image/jpeg" # 例：JPEGファイルの場合
    ...
    with codecs.open(bifile.replace('/01_input', '/03_work').replace('.jpeg', '_ocr_azure.json').replace('.jpg', '_ocr_azure.json'), 'w', 'utf-8') as f2:
...
def characters_comp(azlist, gzlist):
    ...
    for az in azlist:
        ...
        if index is None or gzlist[index][0] == az[0]:
            continue
        else:
            ...
            if idiff == 0:
                ...
                if az[0] in '。｡、､．.\'･,()\"' and not az[23] == 'red':
                    hw = int(min(abs((az[5] or 0) - (az[3] or 0)), abs((az[6] or 0) - (az[4] or 0)))*4.0)
                    za[0] -= hw
                    za[1] -= hw
                    za[2] += hw
                    za[3] += hw
...
def sort_vlist1(zlist):
    ...
    if len(zlist)>0:
        ...
        #row[10]<50本文
        xy = [[ceil(int(row[14]),20), ceil(int(row[15]),20), ceil(int(row[16]),20), ceil(int(row[17]),20), ceil(int(row[16])-int(row[14]), 20)] for row in zlist if row[13]=='horizontal' and row[1]==1 and int(row[10])<=50]
        # xy = list(map(list, set(map(tuple, xy))))
        # #row[10]>=50見出し
        xy2 = [[ceil(int(row[14]),20), ceil(int(row[15]),20), ceil(int(row[16]),20), ceil(int(row[17]),20)] for row in zlist if row[13]=='horizontal' and row[1]==1 and int(row[10])>50]
            ...
            for xrect in x_groups[key]:
                ...
                if current_block is None:
                    ...
                    if abs(x0_r - current_block['x0s'][-1]) <= x_continuity_threshold:
                        last_y0_of_block = max(current_block['y0s']) if current_block['y0s'] else 0
                        if y1_r > last_y0_of_block + y_continuity_threshold or zwidth>most_common_w*2.0:
                            # y方向に大きな隙間がある場合、新しいブロック開始
                            min_y0 = max(current_block['y0s']) if current_block['y0s'] else 0
                            min_x0 = min(current_block['x0s']) if current_block['x0s'] else 0
                            max_y1 = min(current_block['y1s']) if current_block['y1s'] else 0
                            ...
                            most_common_w = w_counts.most_common(1)[0][0] if w_counts else 0
...
for changes in watch(WATCH_DIRECTORY + '/01_input'):
    for action, spath in changes:
        ...
        if ACTION_TYPE_MAP.get(action) == "Added":
            ...
            sorted_list = sorted(addlist)
            for ifile in sorted_list:
                ...
                if '.pdf' in ifile:
                    ...
                    im_rotate.save(memory_buffer, format="JPEG")
                    # メモリバッファを使用して再読み込み
                ...
                if PLAN == "A シングル":
                    print('A:'+ifile)
                    # thread1 = threading.Thread(target=azure, args=(memory_buffer, ifile)) #1
                    # thread1.start() #2
                    # thread1.join() #3
                ...
                if PLAN == "A シングル":
                    ...
                    if os.path.isfile(azure_name) is True:
                        ...
                        for l in range(0, len(alist), 1):
                            ...
                            if  (alist[l][1]==1 or 6<=alist[l][1]) and alist[l][9]<=30 and alist[l-1][13] == 'horizontal'and alist[l][13] == 'horizontal':
                                if alist[l][1]==1:
                                    before=alist[l-1][1]
                                    o = 0

                                #　罫線なし
                                # if (9<=alist[l][1]+o<=14+o and alist[l][0] in "-–ー「]|」") or (alist[l][1]==1 and alist[l][0] in "-–ー「|]」" and alist[l][11]<=0.98) or alist[l][0]=="|":
                                ...
                                #　罫線なし
                                # if (11<=alist[l][1]+o<=14+o and alist[l-1][0] in "、。)・一」"):
                                ...
                                # if (10<=alist[l][1]+o<=14+o and alist[l][11]<=0.7):
                                ...
                                #スペース
                                elif (abs(int(alist[l-1][1] or 0)+int(alist[l-1][2] or 0)-int(alist[l][1] or 0))>=2 and alist[l][1]!=1 and not alist[l][0] in '、()（）「」"“…・' and not alist[l-1][0] in '、()（）「」"“…・') or int(alist[l][24] or 0)>=38: # 
                                    print('space'+ str(l))
                                    p = 1
                                    for n in range(l, len(alist), 1):
                                        if l == n:
                                            p, m1, m2, x0, y0, lx, ly = 1, alist[l][1], alist[l][2], alist[l][3], alist[l][4], alist[l][14], alist[l][15]
                                            alist[l][1], alist[l][14], alist[l][15] = p, x0, y0
                                        else:

                                            if alist[n][1]==1 or (not alist[n][0] in '、()（）「」"“…・' and int(alist[n][24] or 0)>=38):
                                                l=n-1
                                                o=0
                                                break
                                            elif int(m1 or 0)+int(m2 or 0)+p<=int(alist[n][1] or 0)+int(alist[n][2] or 0) and alist[n][14] == lx and alist[n][15] == ly:
                                                m2 = alist[n][2]
                                                if m2>0 :
                                                    alist[n][1], alist[n][14], alist[n][15] = p, x0, y0
                                                elif m2==0:
                                                    alist[n][1], alist[n][14], alist[n][15] = p+1, x0, y0
                                                    p=p+1
                                            # else:
                                            #     l=l+1
                                            #     o=0
                                    ...
                                    # for n in range(l, len(alist), 1):
                                    #     if alist[n][14] == lx and alist[n][15] == ly:
                                    #         # if abs(alist[n-1][1]+alist[n-1][2]-alist[n][1])>=2:
                                    #         #     print('space'+ str(n))
                ...
                # else:
                # テキスト保存
                with open(dname.replace('03_work', '04_text') + '\\' + os.path.splitext(os.path.basename(ifile))[0] + '.txt', mode='w', encoding='utf-8') as f:
                    text = ""
                    for l, x in enumerate(xlist):
                        text = str(xlist[l][0])
                        if l==0 or (l>0 and xlist[l][13] == 'horizontal' and xlist[l-1][13] == 'horizontal' and xlist[l-1][8] <= xlist[l][8])  or (l>0 and xlist[l][13] == 'vertical' and xlist[l-1][13] == 'vertical' and xlist[l-1][7] <= xlist[l][7]):
                            f.write(text)
                        else:
                            f.write('\n' + text)
                ...
                if not xlist is None:
                    for xx in xlist:
                        ...
                        if xx[13] == "horizontal"  and xx[12] == 'r000': #横書
                            ...
                            fontsize = float(max(w, h))/DPI*72*1.0 #0.8係数は次行と重複防止
                            if unicodedata.east_asian_width(xx[0][0:1]) in 'HNa':
                                fontsize = fontsize*1.0
                            pos = pymupdf.Point(round(float(x)/DPI*72, 2), round(float(y)/DPI*72+fontsize, 2))
                            if  mode == 'RGBA':
                            ...
                            if output == "TLT（透明）":
                                fontsize = float(max(w, h))/DPI*72*1.0
                                if unicodedata.east_asian_width(xx[0][0:1]) in 'HNa':
                                    fontsize = fontsize*1.0
                                pos = pymupdf.Point(round(float(x)/DPI*72, 2), round(float(y)/DPI*72, 2))
                                if mode == 'RGBA':
                            ...
                            if output == "TLT（透明）":
                                fontsize = float(h)/DPI*72*1.0
                                if unicodedata.east_asian_width(xx[0][0:1]) in 'HNa':
                                    fontsize = fontsize*1.0
                                pos = pymupdf.Point(round(float(x)/DPI*72, 2), round(float(y)/DPI*72+fontsize*1.0, 2))
                                if  mode == 'RGBA':
                                    page.insert_text(point=pos, text=xx[0], fontname=fontname, fontfile=fontfile, color=cmyk, fontsize=fontsize, rotate=0, stroke_opacity=stroke_opacity, fill_opacity=fill_opacity, overlay=False)
                                else:
                                    page.insert_text(point=pos, text=xx[0], fontname=fontname, fontfile=fontfile, color=rgb, fontsize=fontsize, rotate=0, stroke_opacity=stroke_opacity, fill_opacity=fill_opacity, overlay=False)
                            ...
                            if output == "TLT（透明）":
                                ...
                                if unicodedata.east_asian_width(xx[0][0:1]) in 'HNa':
                                    fontsize = fontsize*1.0
                                pos = pymupdf.Point(round(float(x)/DPI*72+fontsize*1.0, 2)-fontsize*1.0, round(float(y)/DPI*72, 2))
                                if mode == 'RGBA':
                                    page.insert_text(point=pos, text=xx[0], fontname=fontname, fontfile=fontfile, color=cmyk, fontsize=fontsize, rotate=270, stroke_opacity=stroke_opacity, fill_opacity=fill_opacity, overlay=False)
                                else:
                                    page.insert_text(point=pos, text=xx[0], fontname=fontname, fontfile=fontfile, color=rgb, fontsize=fontsize, rotate=270, stroke_opacity=stroke_opacity, fill_opacity=fill_opacity, overlay=False)
                            ...
                            if output == "TLT（透明）":
                                if unicodedata.east_asian_width(xx[0][0:1]) in 'HNa':
                                    fontsize = fontsize*1.0
                                pos = pymupdf.Point(round(float(x)/DPI*72+fontsize, 2), round(float(y)/DPI*72, 2))
                                if  mode == 'RGBA':
                ...
                G_CURRENT_FILE_PATH = ifile
                del ifile, doc, page, file_path, json_open, json_load, img, back_image #, page #azure_name, f, azure_size,

                gc.collect()

                # os.remove(G_CURRENT_FILE_PATH.replace('01_input','06_back_image'))
                # os.remove(G_CURRENT_FILE_PATH.replace('01_input','03_work').replace('.pdf','_ocr.png').replace('.tif','_ocr.png').replace('.tiff','_ocr.png').replace('.png','_ocr.png').replace('.jpg','_ocr.png').replace('.jpeg','_ocr.png'))
                shutil.move(G_CURRENT_FILE_PATH, WATCH_DIRECTORY + '/01_input/completed/' + os.path.basename(G_CURRENT_FILE_PATH))
                ...
                #TLT連結
                print(os.path.splitext(os.path.basename(G_CURRENT_FILE_PATH[0:-10]))[0])
                addlist = glob.glob(WATCH_DIRECTORY + '/02_output/' + os.path.splitext(os.path.basename(G_CURRENT_FILE_PATH[0:-10]))[0] + '*★*_TLT*.pdf')
                ...
                if len(addlist) == ALLPAGE_COUNT and ALLPAGE_COUNT > 1:
                    ...
                    G_CURRENT_FILE_PATH = os.path.splitext(os.path.basename(G_CURRENT_FILE_PATH[:-10]))[0] + '_merge.pdf'
                    doc.save(WATCH_DIRECTORY + '/02_output/' + G_CURRENT_FILE_PATH)
